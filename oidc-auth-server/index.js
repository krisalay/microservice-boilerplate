...
provider.initialize({
  ...
  router.get('/get-token', function* getToken(next) {
    let accessToken = yield provider.AccessToken.find(this.req.headers.authorization);
    if(accessToken){
      let payload = {
        email: "platformadmin@hcm.com",
        role: "PFA"
      };
      ...
      this.body = "Error: you are not authorized to perform this operation";
    }
    yield next;
  });
